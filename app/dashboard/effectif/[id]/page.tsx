...
import { getPlayerAnalysisEvents } from "@/app/tactique/analyse-video/actions"
...
export default async function PlayerFichePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [player, seasonStats, physical, analysisEvents] = await Promise.all([
    getPlayerById(id),
    getPlayerSeasonStats(),
    getPlayerPhysicalStats(id),
    getPlayerAnalysisEvents(id),
  ])
  if (!player) notFound()
  ...
  return <PlayerFicheClient player={player} stats={stats} medical={medical} physical={physical} analysisEvents={analysisEvents} />
}
